...
/////////////////////////////////////////////////////////
    ...
    @IBAction func addAPlayer(sender : UIButton) {
        // pass the game ref to the next user detail view controller so that he can add a player to the gmae
        performSegue(withIdentifier: "toUserDetailVC", sender: sender)
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let tempForPlayerNames = CommonData.sharedInstance.data[CommonData.sharedInstance.gameSelected]?["games"]?.last?.players
        CommonData.sharedInstance.playerNames["copyOfPlayerNames"] = tempForPlayerNames
        
        let k = CommonData.sharedInstance.gameSelected
        if let total = CommonData.sharedInstance.playerNames["copyOfPlayerNames"]?.count {
//            scrollVw = UIScrollView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height))
            ...
            scrollVw.contentSize = CGSize(width: CGFloat(40 + (75*total) + (10*total-1)), height: self.view.frame.height)
...
//                if (CommonData.sharedInstance.playerNames["copyOfPlayerNames"]?.count)! > 0 {
